...
def preprocess_image_details(image_details):
    ...
    if 'annotations' in image_details.keys():
        ...
        for index, annotation in enumerate(image_details['annotations']):
            ...
            vis_bbox = ann_utils.get_visible_bbox(segmentation,
                                                  image_height,
                                                  image_width,
                                                  iscrowd)
            if vis_bbox == None:
...
def get_tfrecord_single_example(image_dict):
    ...
    if 'annotations' in image_dict.keys():
        for annotation in image_dict['annotations']:
            ...
            vis_xmin.append(annotation['vis_bbox'][0])
            vis_ymin.append(annotation['vis_bbox'][1])
            vis_xmax.append(annotation['vis_bbox'][2])
            vis_ymax.append(annotation['vis_bbox'][3])
        ...
        feature_dict_annotation = {
            ...
            'image/object/vis_bbox/xmin':
                tfrecord_utils.float_list_feature(vis_xmin),
            'image/object/vis_bbox/ymin':
                tfrecord_utils.float_list_feature(vis_ymin),
            'image/object/vis_bbox/xmax':
                tfrecord_utils.float_list_feature(vis_xmax),
            'image/object/vis_bbox/ymax':
                tfrecord_utils.float_list_feature(vis_ymax),
...
if __name__ == "__main__":
    ...
    logging.basicConfig(level=logging.INFO,
                        filename=os.path.join(arguments['save_dir'],
                                              'logfile.log'))
    ann_utils.create_coco_labelmap(
        coco_data_dir=arguments['coco_dir'],
        save_dir=arguments['save_dir'],
        coco_year=arguments['coco_year']
    )

    all_images_details = ann_utils.coco_generator(
        arguments['coco_dir'],
        arguments['coco_year'],
        arguments['subset']
    )
